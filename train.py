"""
Train EfficientNet-B0 model for chilli grading.
"""
...
Starting training for {config['epochs']} epochs...")
    ...
    for epoch in range(config["epochs"]):
        ...
        if no_improve_count >= config["early_stopping_patience"]:
            print(f"
No improvement for {config['early_stopping_patience']} epochs. Stopping.")
            break

    total_time = time.time() - total_start
    
    print(f"Training completed in {total_time / 60:.1f} minutes")
    print(f"Validation accuracy: {best_val_acc:.2f}% at epoch {best_epoch + 1}")
